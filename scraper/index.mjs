...
const sitesConfig = [
  ...
  },
  {
    ...
    fieldSelectors: {
      ...
      position: "h2, h3, h4",
      companyName: ".company-name",
      location: "div:nth-child(2) > div:nth-child(2) > p:nth-child(1) > a:nth-child(2)",
      datePosted: "div:nth-child(2) > div:nth-child(2) > p:nth-child(1) > span:nth-child(1)",
      type: ".module-head h1",
    },
...
class ScrollStrategies {
  ...
  static async loadMore(page, config) {
    ...
    let previousCount = 0
    let currentCount = await page.$$eval(config.listingSelector, (el) => el.length)
    let attempts = 0
    ...
    while (attempts < maxAttempts) {
      ...
      try {
        ...
        Logger.warn(`Load more button not found or error clicking it (attempt ${attempts + 1}/${maxAttempts})`)
        attempts++
      ...
      currentCount = await page.$$eval(config.listingSelector, (el) => el.length)
      ...
      return Array.from(document.querySelectorAll(listingSelector))
        .map((el) => {
          const result = {
            ...
            companyLogo: el.querySelector(fieldSelectors.companyLogo)?.src || "N/A",
            ...
            datePosted:
              el.querySelector(fieldSelectors.datePosted)?.getAttribute("datetime") ||
              parseText(el, fieldSelectors.datePosted),
            applicationDeadline: parseText(el, fieldSelectors.applicationDeadline).replace(/Closes:\s*/i, ""),
          }

          // Validate required fields
          return result.link !== "N/A" && result.position !== "N/A" && result.position.length > 2 ? result : null
        })
        .filter(Boolean)
    }, config)
  }
  ...
  static async unicef(page, config) {
    return page.evaluate(({ listingSelector, fieldSelectors }) => {
      ...
      return Array.from(document.querySelectorAll(listingSelector))
        ...
        .filter((job) => job.link !== "N/A" && job.position !== "N/A" && job.position.length > 2)
    }, config)
  }

  static async opportunitiesForYouth(page, config) {
    return page.evaluate(({ listingSelector, fieldSelectors }) => {
      return Array.from(document.querySelectorAll(listingSelector)).flatMap((col) => {
        const sectionTitleEl = col.querySelector(fieldSelectors.type)
        const type = sectionTitleEl ? sectionTitleEl.innerText.trim() : ""
        const articles = Array.from(col.querySelectorAll(fieldSelectors.article))
        ...
        return articles
          .map((article) => {
            ...
            const companyEl = article.querySelector(fieldSelectors.companyName)
            const locationEl = article.querySelector(fieldSelectors.location)
            const datePostedEl = article.querySelector(fieldSelectors.datePosted)
...
class JobScraper {
  ...
  async scrapeSiteWithRetry(config, maxRetries = CONFIG.MAX_RETRIES) {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        Logger.info(`Scraping ${config.name} (attempt ${attempt}/${maxRetries})`)
        const listings = await this.scrapeSite(config)
        this.stats.successfulScrapes++
        return listings
      } catch (error) {
        Logger.error(`Attempt ${attempt} failed for ${config.name}:`, error.message)
  ...
  async scrapeSite(config) {
    ...
    try {
      ...
      Logger.success(`Found ${listings.length} valid listings for ${config.name}`)
  ...
  async scrapeAllSites() {
    try {
      ...
      await mkdir(path.dirname(CONFIG.COMBINED_OUTPUT_FILE), { recursive: true })
  ...
  logFinalStats(allListings) {
    ...
    Logger.info(`Valid URLs: ${allListings.filter((job) => job.isValidUrl).length}`)
...
async function main() {
  ...
  try {
    ...
    Logger.success(`✅ All scraping completed successfully in ${duration} seconds`)
  } catch (error) {
