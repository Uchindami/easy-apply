...
export const useProfileStore = create<ProfileState & ProfileActions>((set, get) => ({
  ...
  toggleWebPushNotifications: () => set((state) => ({ webPushNotifications: !state.webPushNotifications })),
  toggleEmailNotifications: () => set((state) => ({ emailNotifications: !state.emailNotifications })),
  setLoading: (isLoading) => set({ isLoading }),
  setError: (error) => set({ error }),
  updateProfile: async () => {
    const { user, username, socialAccounts, webPushNotifications, emailNotifications } = get()
